...
def create_html():
  mapping_pages_directory = "docs/developers/aixm-5.1.1-to-airm-1.0.0"
  ...
  html = open("data/html/templates/AIXM-list-template.html").read()
  soup = BeautifulSoup(html, "lxml") 

  #For each entry
    #create table entry
  for record in aixm_mapping_merged_dict:
    tr = soup.new_tag("tr")
    print(record)
    print("\n")
    ...
    if str(record["Data Concept"]) != "missing data" and str(record["Data Concept"]) != "":
      url = "aixm-5.1.1-to-airm-1.0.0/"+str(record["Information Concept"])+".html"+"#"+str(record["Data Concept"])
  ...
  f= open("docs/developers/aixm-5.1.1-to-airm-1.0.0.html","w+")
