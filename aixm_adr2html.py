...
def create_html():
  mapping_pages_directory = "docs/developers/aixm-5.1.1-adr-23.5.0-extension-to-airm-1.0.0"
  ...
  import aixm_adr
  aixm = aixm_adr.Aixm_adr()
  aixm_adr_mapping_dict = aixm.aixm_adr_mapping_dataframe.to_dict('records')

  #Create index page
  #creates soup for developers/amxm-2.2.0-to-airm-1.0.0.html using amxm_mapping_template.html
  html = open("data/html/templates/AIXM-ADR-list-template.html").read()
  soup = BeautifulSoup(html, "lxml") 

  #For each entry
    #create table entry
  for record in aixm_adr_mapping_dict:
    tr = soup.new_tag("tr")
    print(record)
    print("\n")
    ...
    if str(record["Information Concept"]) != "missing data" and str(record["Information Concept"]) != "":
      if str(record["Data Concept"]) == "missing data" or str(record["Data Concept"]) == "":
        url = "aixm-5.1.1-adr-23.5.0-extension-to-airm-1.0.0/"+str(record["Information Concept"])+".html"
        text = str(record["Information Concept"])
        #print(text)
    ...
    if str(record["Data Concept"]) != "missing data" and str(record["Data Concept"]) != "":
      url = "aixm-5.1.1-adr-23.5.0-extension-to-airm-1.0.0/"+str(record["Information Concept"])+".html"+"#"+str(record["Data Concept"])
      text = str(record["Data Concept"])
      #print(text)
      new_link = soup.new_tag("a")
      new_link['href'] = url
      new_link['target'] = "_blank"
    ...
    if str(record["Basic Type"]) != "missing data":
      td_type.string = str(record["Basic Type"])
    else:
      td_type.string = "-"
  ...
  f= open("docs/developers/aixm-5.1.1-adr-23.5.0-extension-to-airm-1.0.0.html","w+")
...
def create_html_pages():
  ...
  aixm = aixm_adr.Aixm_adr()
  airm = airm.Airm()
  aixm_adr_info_concepts_dict = aixm.get_information_concepts()

  for info_concept in aixm_adr_info_concepts_dict:
    if info_concept['Information Concept']!="missing data":
      ...
      html = open("data/html/templates/AIXM-ADR-concept-template.html").read()
      soup = BeautifulSoup(html, "lxml") 
      ...
      #span.string = str(info_concept['Information Concept'])
      soup.title.string = str(info_concept['Information Concept'])+" - AIXM 5.1.1 ADR 23.5.0 Extension to AIRM 1.0.0 | AIRM.aero"
